...
class CompatibleRakNet:
    ...
    async def handle_open_connection_request_2(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            logger.info(f"Processing open connection request 2 from {addr} (data length: {len(data)})")
            ...
            # Send open connection reply 2 with proper structure
            response = bytearray()
            ...
            logger.info(f"Preparing response: {response.hex()}")
            
            await self.send_packet(bytes(response), addr)
            logger.info(f"Sent open connection reply 2 to {addr} (size: {len(response)} bytes)")
